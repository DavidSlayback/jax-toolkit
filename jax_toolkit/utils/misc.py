...
"""Utilities for JAX."""
__all__ = ['add_batch_dim', 'batch_concat']
import functools
import itertools
import queue
import threading
...
from ..typing import Nest, NestedArray, PRNGKey, TrainingMetrics, TrainingStepOutput  # pylint: disable=g-multiple-import
import jax
...
N = TypeVar('N', bound=NestedArray)
T = TypeVar('T')


def add_batch_dim(values: Nest) -> NestedArray:
    """Add batch dimension to all components of input"""
...
def _flatten(x: jnp.ndarray, num_batch_dims: int) -> jnp.ndarray:
    ...
    Returns:
      flattened input.
    """
    if x.ndim < num_batch_dims: return x
...
def _fetch_devicearray(x):
    if isinstance(x, jax.xla.DeviceArray): return np.asarray(x)
    return x


def batch_to_sequence(values: Nest) -> NestedArray:
    return jax.tree_map(lambda x: jnp.transpose(x, axes=(1, 0, *range(2, len(x.shape)))), values)
...
def tile_nested(inputs: Nest, multiple: int) -> Nest:
    """Tiles tensors in a nested structure along a new leading axis."""
    # tile = functools.partial(jnp.stack, [array] * multiple)
...
_SplitFunction = Callable[[NestedArray], PrefetchingSplit]


def sharded_prefetch(
        iterable: Iterable[NestedArray],
        buffer_size: int = 5,
        num_threads: int = 1,
        split_fn: Optional[_SplitFunction] = None,
        devices: Optional[Sequence[jax.xla.Device]] = None,
) -> Generator[NestedArray, None, None]:
